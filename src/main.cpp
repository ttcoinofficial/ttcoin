...
#if defined(NDEBUG)
# error "TTcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xca73a393f3899261f8ac6333f0aa3e49c39d4bf3d696105971eff2c52eb9dfbb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TTcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TTcoin Signed Message:\n";
...
{
    // TTcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // TTcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20.5 * 60; // TTcoin: 20.5 minutes
static const int64 nTargetSpacing = 2.5 * 60; // TTcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TTcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TTcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // TTcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xca73a393f3899261f8ac6333f0aa3e49c39d4bf3d696105971eff2c52eb9dfbb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 24/Sept/2017 N.F.L. Players and Owners Unite in Face of Trump Tweets";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("311219880fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1506295074;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2096144;
        ...
        assert(block.hashMerkleRoot == uint256("0x1f7ea40a8d6b8d9d983a3bc068ad7ace104c977ec858c3ff9ed69dc8969fc0fa"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TTcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TTcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TTcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TTcoinMiner(CWallet *pwallet)
{
    printf("TTcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("TTcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TTcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TTcoinMiner, pwallet));
}
